...
首先因为 go 团队的 less is more 理念，让编译器做分析调用链这么重的活也不太现实（这里也辩解一下即使调用链分析也无法覆盖反射、传 interface 调用、[插件模式](https://pkg.go.dev/plugin)等场景，所以调用链分析是不现实的）。
