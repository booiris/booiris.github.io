...
> 在 go 中可以理解为 `Unit(x).FlatMap( f )` 的执行结果和执行 `f(x)` 结果xiang

1. 转换子是组合子的右[单位元](https://en.wikipedia.org/wiki/Identity_element): M{ val: x }.FlatMap( Unit ) 等价于 `M< val: x >`
2. 组合子满足结合律:
