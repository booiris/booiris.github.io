...
可以看到代码的嵌套层级非常深，这就是早期 javascript 异步编程的问题，对于异步函数需要传入一个回调函数表明在当前状态结束后 (如读取文件结束后) 应该继续执行的动作。可以想象一旦异步处理过程多了，如果没有合适的方法，必然会导致深层次的函数嵌套。一般的做法就是将嵌套的函数抽出来，将yi bu
...
在 2015 年后，promise 的出现缓解了 javascript 在异步编程中的问题。
